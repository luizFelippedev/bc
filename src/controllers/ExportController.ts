...
export class ExportController {
   ...
   */
  public async exportProjects(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const filters = this.buildFilters(req.query, [
        'category', 'status', 'featured', 'visibility', 'isActive'
      ]);
   ...
   */
  public async exportCertificates(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const format = (req.query.format || 'json') as 'json' | 'csv' | 'xlsx';
      const fields = req.query.fields ? (req.query.fields as string).split(',') : undefined;
      const fileName = req.query.fileName as string;
   ...
   */
  public async downloadExportedFile(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
