...
export class ProjectController {
  ...
  public async createProject(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (req.files) {
        ...
        projectData.images = {
          featured: uploadResults.find(file => file.fieldname === 'featured')?.url || '',
          gallery: uploadResults.filter(file => file.fieldname === 'gallery').map(file => file.url)
        };
      }
      ...
      res.status(201).json(
        ApiResponse.success(project, 'Projeto criado com sucesso')
      );
  ...
  public async updateProject(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (req.files) {
        ...
        projectData.images = {
          ...project.images,
          featured: uploadResults.find(file => file.fieldname === 'featured')?.url || project.images.featured,
          gallery: [
            ...project.images.gallery,
            ...uploadResults.filter(file => file.fieldname === 'gallery').map(file => file.url)
          ]
        };
      }
  ...
  public async deleteProject(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      res.json(
        ApiResponse.success(null, 'Projeto excluído com sucesso')
      );
  ...
  public async toggleFeatured(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      res.json(
        ApiResponse.success({ featured: project.featured }, 'Status de destaque atualizado')
      );
    } catch (error) {
