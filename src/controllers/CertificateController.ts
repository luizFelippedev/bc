...
export class CertificateController {
  ...
  public async getCertificate(
    ...
    try {
      ...
      if (!certificate) {
        res
          .status(404)
          .json(ApiResponse.error('Certificado não encontrado', 404));
        return;
      }
  ...
  public async createCertificate(
    ...
    try {
      ...
      if (req.file) {
        const uploadResult = await this.fileUploadService.uploadSingle(
          req.file
        );
      ...
      this.logger.info(
        `Certificado criado: ${certificate.title} (${certificate._id})`
      );
    } catch (error) {
  ...
  public async updateCertificate(
    ...
    try {
      ...
      this.logger.info(
        `Certificado atualizado: ${certificate.title} (${certificate._id})`
      );
    } catch (error) {
  ...
  public async deleteCertificate(
    ...
    try {
      ...
      res.json(ApiResponse.success(null, 'Certificado excluído com sucesso'));

      this.logger.info(
        `Certificado excluído: ${certificate.title} (${certificate._id})`
      );
    } catch (error) {
