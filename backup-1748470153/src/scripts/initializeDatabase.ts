...
export class DatabaseInitializer {
  ...
  public static async initialize(): Promise<void> {
    try {
      this.logger.info(
        'Verificando necessidade de inicialização do banco de dados...'
      );
      ...
      if (userCount === 0) {
        this.logger.info(
          'Banco de dados vazio. Iniciando processo de inicialização...'
        );
        ...
        this.logger.info(
          'Banco de dados já possui dados. Inicialização não necessária.'
        );
      }
    } catch (error) {
  ...
  private static async createSampleProjects(): Promise<void> {
    try {
      const sampleProjects = [
        {
          ...
          shortDescription:
            'API completa com autenticação JWT, validação, cache e documentação Swagger.',
          fullDescription:
            'Projeto completo de uma API RESTful desenvolvida com Node.js, Express e MongoDB. Implementa autenticação JWT, validação de dados, cache com Redis, documentação Swagger e testes automatizados.',
          category: 'web_app',
          status: 'deployed',
          visibility: 'public',
          featured: true,
        ...
        },
        {
          ...
          shortDescription:
            'Interface moderna em React com Material-UI e integração com APIs.',
          fullDescription:
            'Aplicação frontend moderna desenvolvida com React, TypeScript e Material-UI. Implementa gerenciamento de estado com Redux, roteamento com React Router, integração com APIs RESTful e GraphQL.',
          category: 'web_app',
          status: 'deployed',
          visibility: 'public',
          featured: true,
      ...
      this.logger.info(
        `✅ ${sampleProjects.length} projetos de exemplo criados com sucesso`
      );
    } catch (error) {
  ...
  private static async createSampleCertificates(): Promise<void> {
    try {
      ...
      this.logger.info(
        `✅ ${sampleCertificates.length} certificados de exemplo criados com sucesso`
      );
    } catch (error) {
