  ...
  public async trackEvent(options: TrackEventOptions): Promise<void> {
    try {
      ...
      if (options.eventType === 'project_view' && options.projectId) {
        await Project.findByIdAndUpdate(options.projectId, {
          $inc: { views: 1 },
        });
      }
    ...
    try {
      ...
      if (!stats) {
        ...
        const recentProjectViews = await Analytics.find({
          ...
          createdAt: { $gte: fifteenMinutesAgo },
        }).populate<{ projectId: { title: string; slug: string } }>(
          'projectId',
          'title slug'
        );
        ...
        stats = {
          ...
          pageViews: recentEvents.filter((e) => e.eventType === 'page_view')
            .length,
          projectViews: recentEvents.filter(
            (e) => e.eventType === 'project_view'
          ).length,
          recentProjects: recentProjectViews
            .map((e) => ({
              title: e.projectId?.title,
              slug: e.projectId?.slug,
  ...
  private calculateStats(events: any[]): any {
    ...
    return {
      ...
      byType: {
        ...
        projectView: events.filter((e) => e.eventType === 'project_view')
          .length,
        certificateView: events.filter(
          (e) => e.eventType === 'certificate_view'
        ).length,
